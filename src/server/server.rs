...
use hyper::{Request, Response, Server, StatusCode, Body};
...
impl SlackAppServer {
    pub async fn start(listen_addr: SocketAddr, verif_token: String) {
        ...
        let state: &'static SlackAppServerState = Box::leak(Box::new(SlackAppServerState{
           verif_token,
        }));
    ...
    async fn try_process_request(state: &SlackAppServerState, req: Request<Body>) -> Result<Response<Body>, Box<dyn Error + Send + Sync>> {
        ...
        let event_type = match json.get("type") {
            ...
            _ => return_error!(StatusCode::BAD_REQUEST, "Missing or invalid type field in request"),
        };
        ...
        match event_type {
            ...
            _ => {
                Err(format!("Received unhandled event: {}", json).into())
            },
        }
    }

    async fn slack_service(state: &SlackAppServerState, req: Request<Body>) -> Result<Response<Body>, Infallible> {
